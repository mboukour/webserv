...
int hex_to_decimal(const char *hex_str) {
    int result = 0;
    std::cout << BLUE << hex_str << RESET << std::endl;
    while (*hex_str) {
        char c = *hex_str;
        result *= 16;
        if (isdigit(c))
...
long get_Lchuncked(std::string body)
{
    std::string tmp;
    for(long i = 0 ; i < (long)body.length();i++)
    {
        if (body[i] == '\r' && body[i+1] == '\n')
        {
            i +=2;
            while(i < (long)body.length() && body[i] != '\r' && body[i+1] != '\n')
                tmp.push_back(body[i++]);
            if(!tmp.empty())
                return hex_to_decimal((char *)tmp.c_str());
        }
    }
    return 0;
}
    ...
    if (__contentType ==
      ...
      std::cout << MAGENTA << "Multi Form Data" << RESET << std::endl;
        ...
        if (this->postState == INIT_POST){
            ...
            this->size += get_Lchuncked(request.getReqEntry());
            if (this->postState == LAST_ENTRY){
                postResponse(request, 500, "Error", "NOT UPLOADED");
                std::cout << GREEN << this->size << RESET << std::endl;
            }
        }
  }
}
